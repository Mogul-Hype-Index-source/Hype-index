...
def build_index_payload(scored: List[Dict[str, Any]],
    ...
    for m in scored:
        ...
        public_movies.append({
            ...
            "rating_band":    m.get("rating_band", "UNRATED"),
            "score":          m.get("score", 0),
            "move":           m.get("move", 0),
    ...
    return {
        ...
        "summary": {
            "total":              len([m for m in public_movies if m.get("release_type") != "streaming"]),
            "total_streaming":    len([m for m in public_movies if m.get("release_type") == "streaming"]),
        ...
        "movies":      [m for m in public_movies if m.get("release_type") != "streaming"],
